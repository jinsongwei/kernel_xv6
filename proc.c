...
unsigned int tickets_base = 0;
unsigned int p_win = 0;
unsigned int c_win = 0;
...
char lot_child[16] = "lot_child";
char lot_parent[16] = "lot_parent";
            ...
            if(i != 1){
                ...
                while(run_p[i] != 0){
                    if(compare_string(run_p[i]->name,lot_child) ||
                            compare_string(run_p[i]->name,lot_parent)){
                        cprintf("---proc %d, %s runnable---\n", run_p[i]->pid,run_p[i]->name);
                    }else{
                        cprintf("## proc %s ##\n",run_p[i]->name);
                    }
                    i++;
                ...
                unsigned int j;
                for(i=0;i<run_num;i++){
                    for(j=i+1;j<run_num;j++){
                        if(run_p[i]->pid == run_p[j]->pid){
                            run_p[j] = 0;
                        }
                    }
                }
